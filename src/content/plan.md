...
- **Download clients (qBittorrent/Deluge/Prowlarr) on their own isolated, VPN-routed VLAN** — segregated from the house network and forced out through a VPN + kill-switch so their traffic never mixes with everything else (see Operations → Network scheme).
...
- **Define the VLAN/IP scheme now** (IoT / voice / CCTV / guest / main / downloads), **non-overlapping with Selby** (e.g. `10.10.x` here, `10.20.x` there) — it's a prerequisite for the SD-WAN, guest isolation and the camera egress-blocking the privacy plan relies on.
- **📥 Download/*arr stack on its own isolated, VPN-routed VLAN.** **qBittorrent, Deluge and Prowlarr** (plus the *arr apps) run as Docker containers on Unraid bound to a dedicated **`downloads` VLAN** — a VLAN-tagged macvlan/ipvlan network, not Unraid's normal bridge. Firewall it so it **can't reach any other VLAN** (no HA, cameras, main), and **force its WAN egress through a VPN with a kill-switch** (e.g. **gluetun**, or the binhex `*vpn` images) so nothing leaks if the tunnel drops. The UCG-Ultra carries the policy route and firewall rules; the rest of the house never sees that traffic. *(Caveat: the *arr apps still need to reach the media shares + the download clients — keep those flows on the same VLAN/host so the segmentation doesn't break imports.)*
