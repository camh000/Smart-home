...
export const SHOPPING: ShopPhase[] = [
  ...
  },
  {
    ...
    items: [
      { id: "b8", name: "16-port UniFi PoE+ switch", meta: "UniFi USW-Lite-16-PoE — single-pane with the UCG-Ultra. PoE budget note: once WiFi-7 UniFi APs replace the Decos they draw PoE too, so consider sizing up (e.g. USW-Pro-Max-16-PoE, which also adds the 2.5G ports the Ultra's gigabit LAN lacks).", cost: 200, costLabel: "~£180-220", search: "Ubiquiti UniFi USW-Lite-16-PoE" },
      ...
      { id: "b12", name: "Smart lock — Front door", meta: "Pick by door type: Yale Linus L2 (euro retrofit, keeps key) or Conexis L2 (multipoint) for a UK uPVC/composite door; Yale Assure 2 only if it's a deadbolt door. Face-rec auto-unlock.", cost: 250, costLabel: "~£250", search: "Yale smart lock" },
      ...
      { id: "b16s1", name: "Mains-interlinked smoke/heat/CO alarms (Grade D) + HA bridge", meta: "Compliant life-safety base (BS 5839-6) — Ei Electronics mains-interlinked + a relay/contact module into HA. Optional extra Zigbee (Frient/Heiman) units as a sensing layer.", cost: 220, costLabel: "~£180-260", search: "Ei Electronics interlinked smoke alarm" },
      { id: "b16s1b", name: "Type 2 SPD (consumer unit)", meta: "Whole-house surge protection — sparky fits in the board; protects every always-on device.", cost: 50, costLabel: "~£30-60", search: "Type 2 surge protection device consumer unit" },
      { id: "b16s1c", name: "PoE surge arrestors (outdoor cams)", meta: "Earthed gas-discharge arrestors on the outdoor camera runs at the head-end — stops a surge frying the switch + Unraid.", cost: 40, costLabel: "~£40", search: "PoE surge protector gigabit" },
      ...
      { id: "b16s4", name: "Sonoff Hydro ONE — water valve + flow meter", meta: "Zigbee valve with a built-in flow meter (local via ZHA/Z2M). The whole-home flow telemetry the stopcock lacks → feeds Water Monitor for dribble / running-toilet / burst detection.", cost: 50, costLabel: "~£50", search: "Sonoff Hydro ONE water valve" },
      { id: "b16s5", name: "Water Monitor (HACS)", meta: "Free integration: continuous-low-flow + tank-refill detection on any flow sensor, drives the stopcock to auto-shut. Local-first (vs US-only Phyn/Hydrific).", cost: 0, costLabel: "free", url: "https://github.com/markaggar/Water-Monitor" },
      { id: "b16s6", name: "Aqara natural-gas detector", meta: "Gas-heated house: methane Zigbee detector near the hob/boiler (local via ZHA/Z2M) → Alarmo + Claude. Complements the smoke/CO units.", cost: 40, costLabel: "~£40", search: "Aqara gas detector JT-BZ-01AQ" },
    ],
  },
  {
    id: "3",
    ...
    items: [
      ...
      { id: "b15b", name: "TV bias light (WLED strip)", meta: "Addressable strip behind the TV — wall glow, cinema-mode dim. Same WLED kit as the bar.", cost: 25, costLabel: "~£25", search: "WS2812B addressable LED strip TV backlight" },
      { id: "b15c", name: "TV screen-sync (Ambilight)", meta: "Picture extends onto the wall. DIY HyperHDR + HDMI capture (~£70, local) or Hue Play HDMI Sync Box + strip (~£250-300, passes 4K Dolby Vision). Cheap Govee/Lytmi kit ~£70-100.", cost: 80, costLabel: "~£70-300", search: "HDMI sync box ambient TV backlight" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "b29al1", name: "Alarmo (HACS alarm panel)", meta: "Turns the presence/CCTV/lock kit into a full intruder alarm — arm modes, delays, code/NFC arming. Config only.", cost: 0, costLabel: "free", url: "https://github.com/nielsfaber/alarmo" },
      { id: "b29al2", name: "Zigbee indoor siren", meta: "Audible alarm output (e.g. Frient/Develco). Voice PEs can also sound it.", cost: 25, costLabel: "~£25", search: "Zigbee siren" },
      { id: "b29al3", name: "External sounder / bell box + strobe", meta: "Visible deterrent on the wall — a house that looks alarmed gets skipped.", cost: 40, costLabel: "~£30-50", search: "alarm bell box strobe" },
      { id: "b29al4", name: "Zigbee arming keypad (optional)", meta: "By the door for code arm/disarm — or arm via tablet/NFC/phone instead.", cost: 25, costLabel: "~£25", search: "Zigbee alarm keypad" },
      { id: "b29al5", name: "4G/LTE WAN failover modem", meta: "USB/Ethernet 4G on the UCG-Ultra — alerts still escape if broadband is cut (the DIY-alarm weak point).", cost: 60, costLabel: "~£40-80", search: "4G LTE USB modem router failover" },
      { id: "b29al6", name: "Zigbee glass-break sensors (×2-3)", meta: "Acoustic — catch a smashed window that contact sensors miss. Vulnerable ground-floor panes.", cost: 60, costLabel: "~£60", search: "Zigbee glass break sensor" },
      { id: "b29al7", name: "Vibration / tilt sensors (×3-4)", meta: "Aqara — garage door, sheds, vulnerable windows; flag forced entry before it opens.", cost: 40, costLabel: "~£40", search: "Aqara vibration sensor" },
      { id: "b29al8", name: "Panic button (bedside / by door)", meta: "Zigbee button → instant full alarm. Also covers a duress-trigger.", cost: 15, costLabel: "~£15", search: "Aqara wireless switch" },
      ...
      { id: "b29k1", name: "ESP32 Bluetooth proxies (×3-4) — Bermuda", meta: "Distributed BT proxies become a per-person room-presence mesh via Bermuda; also serve the Mi Flora plant sensors later. One per main area for coverage.", cost: 60, costLabel: "~£60", search: "ESP32 development board" },
      { id: "b29k2", name: "Bermuda + Private BLE Device", meta: "Free (HACS + core): per-person room location from the BT proxies. iPhones need a one-time IRK capture (ESP32 method or a paired Mac); no iOS iBeacon-TX, so add beacons for snappier presence.", cost: 0, costLabel: "free", url: "https://github.com/agittins/bermuda" },
      { id: "b29k3", name: "BLE beacons (×2-4)", meta: "Cheap always-on tags in bags/pockets/keys/pet collar — snappier and more reliable than power-saving iPhones, plus 'where are my keys' + pet-aware alarm suppression.", cost: 20, costLabel: "~£20", search: "iBeacon BLE beacon tag" },
      { id: "b29k4", name: "Seeed 60 GHz health radar (×2)", meta: "MR60BHA2 (bedside vitals/sleep) + MR60FDA2 (ceiling fall detection). ESPHome-native, fully local — sleep-aware automations + camera-free fall alerts.", cost: 40, costLabel: "~£40", search: "Seeed MR60BHA2 mmWave" },
    ],
  },
  {
    id: "5",
    ...
    items: [
      ...
      { id: "b29r1", name: "Second UPS — network gear", meta: "Keeps UCG-Ultra + core switch (SD-WAN/Tailscale) up during a server shutdown", cost: 90, costLabel: "~£90", search: "APC Back-UPS 650VA" },
      ...
      { id: "b29s", name: "ALPR — Frigate native LPR (0.16+)", meta: "On-device plate recognition built into Frigate — no separate container. CodeProject.AI / Plate Recognizer optional fallbacks. Free.", cost: 0, costLabel: "free" },
      ...
      { id: "b29v1", name: "EMHASS — energy optimiser", meta: "Free: LP/MPC solver for cost/CO2-optimal load scheduling (deferrable loads now; battery/PV later). Claude explains/overrides it. Docker on Unraid.", cost: 0, costLabel: "free", url: "https://emhass.readthedocs.io/" },
      { id: "b29v2", name: "HA Octopus Energy (BottlecapDave, HACS)", meta: "Free: saving sessions, free-electricity sessions, intelligent dispatch + 'cheapest N-hour window' target-rate sensors. The data layer under EMHASS.", cost: 0, costLabel: "free", url: "https://bottlecapdave.github.io/HomeAssistant-OctopusEnergy/" },
      { id: "b29v3", name: "Local-LLM fallback (Qwen3 via Ollama)", meta: "Free: small tool-calling model as the offline brain when the Anthropic API/internet is down. Reuses the Unraid Ollama host; hardens the cloud-reasoning SPOF.", cost: 0, costLabel: "free", url: "https://www.home-assistant.io/integrations/ollama/" },
      { id: "b29v4", name: "AirGradient ONE (optional)", meta: "Reference PM2.5 + true NDIR CO2 + NOx, Works-With-HA. Add an outdoor Open Air for indoor/outdoor delta. Open-hardware alternative to Airthings.", cost: 150, costLabel: "~£150", url: "https://www.airgradient.com/" },
    ],
  },
  {
    id: "6",
    ...
    items: [
      ...
      { id: "b47a", name: "Cinema extras — bass shaker / star ceiling (optional)", meta: "Tactile transducer under the sofa off the AVR LFE (~£60-120); DIY fibre-optic/LED-twinkle star ceiling (~£40-100). Decoration-time wow.", cost: 100, costLabel: "~£100-220", search: "tactile bass shaker transducer" },
      ...
      { id: "b49", name: "Bluetooth Proxy ESP32 (plant sensors)", meta: "Reads BT plant sensors. The same proxies also power Bermuda room presence (see Phase 4).", cost: 20, costLabel: "~£20", search: "ESP32 development board" },
      ...
      { id: "b35", name: "Robot vacuum (Valetudo-compatible)", meta: "Pick a still-rootable model BEFORE buying (Dreame best-supported). Valetudo firmware = cloud-free, LAN-only, per-room MQTT entities Claude can target. Rooting voids warranty.", cost: 350, costLabel: "~£300-450", search: "Dreame robot vacuum" },
