...
export const ROOMS: Room[] = [
  {
    ...
    items: [
      ...
      { cat: "display", label: "TV — TCL 65C6KS (QD-Mini LED, 2025)", detail: "Main lounge display (~£529). Mini-LED suits the bright south-facing room (brighter + no burn-in vs OLED). Entry-tier so likely 60Hz — fine for film + Steam Deck streaming (~60fps), with ALLM/Game Mode. eARC to the AVR; Dolby Vision/Atmos, AirPlay 2. HA control via the Android TV / Google Cast integration + CEC; drives cinema mode, auto-dim, doorbell-feed-to-TV. Apple TV stays the primary source; one-touch CEC wakes TV + AVR when the Apple TV turns on." },
      ...
      { cat: "display", label: "Wall tablet — phase 4", detail: "AV control, scene selection, doorbell feed, ambient info while watching TV." },
      { cat: "display", label: "Steam Deck dock (game streaming)", detail: "Owned JSAUX dock (has Ethernet). Docked Steam Deck streams the office gaming PC to the lounge TV/AVR. Wire the dock's Ethernet port to a lounge Cat6 drop (bypasses the Deck's Wi-Fi) and keep it on the same VLAN as the gaming PC. Drives the “lounge gaming” scene (AVR + TV input + lights + Wake-on-LAN of the PC) — work-hours guarded." },
  ...
  },
  {
    ...
    items: [
      ...
      { cat: "display", label: "Dual monitors + KVM (34\" ultrawide)", detail: "Two monitors — one a 34\" 21:9 ultrawide (3440×1440, 144-165Hz) — shared between the work laptop and gaming PC via a basic dual-head DP 1.4 KVM (manual switch — chosen, no HA control). Buy-time check still open: confirm the work laptop can drive two external displays over USB-C without DisplayLink (locked-down = no driver install)." },
